...
void turn(struct angle *spin, int step ){
    ...
    spin->yaw = yawDist * sinf( 2 * M_PI * yawState/yawPeriod );
    spin->pitch = pitchDist * sinf( 2 * M_PI * pitchState/pitchPeriod );
    spin->roll = rollDist * sinf( 2 * M_PI * rollState/rollPeriod );
